...
**Сначала зарегистрируйте аккаунт.** Повышение ищет по нику, поэтому если
указать ник, который ещё никем не занят, а регистрация на сервере открыта, то
повышен будет тот, кто займёт этот ник первым. Сначала регистрация, потом
переменная — либо задавайте её при закрытых регистрациях. Пока такого аккаунта
нет, api пишет об этом предупреждение на каждом старте.
...
### Метрики (опционально, по умолчанию выключено)

Задайте `METRICS_TOKEN` длинной случайной строкой — и API начнёт отдавать
Prometheus-метрики на `GET /metrics` тем, кто присылает
`Authorization: Bearer <токен>`; не задан — роут не регистрируется вовсе.
Подробности и то, чего в выводе намеренно НЕТ, — в
