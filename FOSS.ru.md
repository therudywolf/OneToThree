...
Пароль пользователя никогда не передаётся на сервер. Аутентификация строится на цифровой подписи challenge:
...
**Чего сервер не видит:** private keys, PIN/passphrase и содержимое vault в открытом виде.

### Шифрование vault

Vault содержит приватные ключи ECDSA и ECDH и шифруется пользовательским PIN или passphrase:
...
В direct chats отправитель и получатель получают общий секрет через ECDH, после чего сообщение шифруется алгоритмом AES-GCM-256 на стороне клиента.

### Распределение группового ключа

В группах создаётся общий симметричный ключ, который упаковывается для каждого участника индивидуально через ephemeral ECDH и AES-GCM.

### Шифрование файлов

Файлы шифруются на клиенте до загрузки в MinIO, а сервер оперирует только presigned URL и зашифрованными blobs.

### Защита звонков

Звонки используют WebRTC, а сервер участвует только в сигналинге. Даже при TURN-релее через coturn медиапоток остаётся зашифрованным с помощью DTLS-SRTP.
...
- ✅ HSTS с preload и длительным max-age.
...
- WebAuthn largeBlob поддерживается не везде, используется fallback.
...
Это один из ключевых разделов: One To Three работает по модели **zero-trust server**.
...
- ❌ Локально расшифрованный кэш пользователя.

### Если сервер скомпрометирован

При полном доступе к БД атакующий получает в основном метаданные: usernames, timestamps, membership и другую служебную информацию, но не содержимое сообщений и файлов в открытом виде.

Итоговая модель безопасности здесь следующая: **конфиденциальность содержимого сохраняется даже при полном компромиссе сервера**, но метаданные общения остаются видимыми.
...
`docker-compose.prod.yml` описывает 7 сервисов: `caddy`, `web`, `api`, `db`, `db-migrate`, `minio` и `coturn`.
...
TURN-хост **обязательно** должен быть в режиме **DNS only**, потому что orange-cloud proxy не поддерживает UDP-трафик, необходимый для WebRTC-звонков.
...
- [x] Background Sync для офлайн-очереди.
...
- [ ] **Гранулярные права администраторов групп** — role system уже есть, детализация ещё впереди.
...
В документе описаны основные сущности: `users`, `devices`, `pushSubscriptions`, `chats`, `chatMembers`, `messages` и `messageDeliveries`, включая хранение публичных ключей, ciphertext-полей, membership-структуры, device state и delivery tracking.

Это отражает основную архитектурную идею проекта: сервер знает структуру коммуникации и служебные метаданные, но не видит содержимое сообщений в открытом виде.
...
**Вариант A: Capacitor** — рекомендованный следующий шаг, позволяет обернуть текущий Next.js-код в native WebView, оценивается примерно в 4–6 недель и открывает доступ к CallKit, Foreground Service и системному contact picker.

**Вариант B: React Native** — полный перенос UI в нативный стек, 4–6 месяцев работы, но с лучшим native UX.

**Вариант C: Flutter** — полная перепись на Dart, 6–9 месяцев, единая кодовая база для нескольких платформ.

---

## Лицензия

OneToThree — свободное программное обеспечение: вы можете распространять и/или
модифицировать его на условиях **GNU Affero General Public License v3.0**,
опубликованной Free Software Foundation.

Это **копилефт**-лицензия. Если вы модифицируете OneToThree и запускаете
изменённую версию как сетевой сервис, вы **обязаны** предоставить полный
исходный код вашей изменённой версии всем пользователям этого сервиса на тех же
условиях AGPLv3.

- SPDX-идентификатор: `AGPL-3.0-only`
