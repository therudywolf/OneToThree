...
- **E2EE-сообщения** — личные чаты на **Double Ratchet (v2) + X3DH** (forward
  secrecy, сессии на каждое устройство); группы — общий секторный ключ. Сервер
  видит только шифртекст.
- **Голосовые и видеозвонки** — WebRTC с TURN-релеем + опциональный LiveKit SFU,
  DTLS-SRTP, мониторинг качества, origin-safe режим.
- **Медиа** — зашифрованная загрузка фото/голоса/видео/файлов + альбомы в
  MinIO/S3, расшифровка на клиенте, локальный кеш в стиле WhatsApp с LRU-вытеснением.
- **Стикеры и GIF** — импорт паков из Telegram, **создание своих паков**
  (загрузка своих картинок), анимированные (tgs/lottie) стикеры, поиск GIF
  (Tenor/Giphy) + избранное, нативный эмодзи-пикер.
- **Группы и каналы** — зашифрованное распределение группового ключа; опросы,
  реакции, ответы, поиск по сообщениям (локально, по расшифрованному тексту).
- **Мультиустройство** — привязка по QR, восстановление по фразе, отзыв доступа.
- **2FA** — опциональный TOTP (RFC 6238).
- **Гостевые ссылки** (по желанию, `FEATURE_GUESTS`) — позвать человека без
  аккаунта: **ссылка на встречу** пускает нескольких гостей, каждого вы
  впускаете лично (аккаунт им не заводится вовсе), или **временный чат**,
  который живёт, пока у гостя открыта вкладка. Подробнее —
  [GUEST_MODE_CONCEPT.ru.md](./docs/project/GUEST_MODE_CONCEPT.ru.md).
- **Клиенты** — устанавливаемый PWA (веб) с Web Push (VAPID), нативный
  **Android** (Capacitor) и **десктоп** (Tauri — Windows/macOS/Linux) — всё из
  одного Next.js-бандла.
- **Самохостинг** — одна команда `./startup.sh`, авто-TLS через Let's Encrypt.
  Можно собрать свой десктоп/Android под свой сервер ([Десктоп](#десктоп-приложение)).
...
контрольную сумму и открой всё равно, либо собери сам (ниже). Либо просто открой веб-приложение (устанавливаемая PWA). Все
...
2. Зарегистрируйте новый аккаунт — повышение ниже аккаунт не создаёт
3. Укажите этот ник в окружении api и перезапустите api:
...
На старте api повысит его до группы `creator`, но только пока на сервере нет ни
одного создателя — дальше переменная безвредна, её можно оставить.

То же руками (выставляйте **оба** поля — `role` открывает панель, а
`user_group` проверяет каждое привилегированное действие):
...
4. Откройте `/admin` (нужно быть авторизованным). Вкладка **CONFIG** там
   показывает живую сборку, доступность Postgres/Redis/LiveKit и ручки,
   читаемые на каждый запрос (открытая регистрация, TTL гостевой ссылки, места
   во встрече, потолки гостей) — каждая поверх своей переменной окружения и без
   перезапуска.
...
Нативное десктоп-приложение (Windows / macOS / Linux) на **Tauri** оборачивает
тот же Next.js-бандл, что и веб/Android.

### Готовая сборка

Установщики приложены к каждому [GitHub Release](https://github.com/therudywolf/OneToThree/releases) (например `OneToThree_x.y.z_x64-setup.exe` для Windows), рядом `.sha256`.

### Сборка под свой сервер (самохостинг)

По умолчанию сборка смотрит на публичный инстанс. Чтобы нацелить её на **свой**
сервер с корректным CSP — без правок кода:

```bash
cd desktop/tauri
cp .env.example .env          # задай OT_API_URL / OT_APP_URL / OT_S3_URL / OT_LIVEKIT_URL
npm install
npm run build:selfhost -- --bundles nsis    # или deb / appimage / dmg под свою ОС
```

`build:selfhost` читает твои хосты и флаги (`OT_ENABLE_CALLS`, `OT_ENABLE_GIF`) из `.env`, пересобирает из них CSP-разрешения и кладёт установщик в `desktop/tauri/src-tauri/target/release/bundle/`.

### Сборка под публичный инстанс

```bash
cd desktop/tauri && npm install && npm run build          # или build:bundles
```
