...
| 2026-04-22 | **Ops/Deploy helper** — `start.sh` получил отдельную команду `./start.sh tg`: интерактивно запрашивает `TELEGRAM_BOT_TOKEN`, сохраняет в `.env.prod` + `secrets/telegram_bot_token`, затем делает `api --force-recreate`; в `./start.sh update` добавлен soft-prompt токена, если он не задан. |
