...
export function ActiveCallOverlay({
  ...
  function handleLocalMouseDown(e: React.MouseEvent) {
  ...
  useEffect(() => {
    ...
    function handleMouseMove(e: MouseEvent) {
      ...
      setLocalDragPos({
        x: Math.max(0, Math.min(window.innerWidth - 150, dx + 10)),
        y: Math.max(0, Math.min(window.innerHeight - 200, dy + 10)),
      })
    }
        ...
        <>
          <div className="flex shrink-0 items-center justify-between border-b border-red-500/50 px-4 pb-3 pt-[max(0.75rem,env(safe-area-inset-top))] shadow-[0_0_10px_rgba(255,0,0,0.35)]">
            ...
            <button
              ...
              className="px-3 py-1 text-xs underline text-neon-cyan hover:text-neon-cyan/80"
            >
              BACK
            </button>
          </div>
          <div className="min-h-0 flex-1 flex items-center justify-center">
            {remoteStreams[fullscreenPeerId] && (
              <PeerTile
                peerId={fullscreenPeerId}
                stream={remoteStreams[fullscreenPeerId]}
                label="REMOTE"
                isFullscreen={true}
                onFullscreenToggle={() => setFullscreenPeerId(null)}
                remoteMicMuted={remotePeerMedia[fullscreenPeerId]?.micMuted}
                remoteCamOff={remotePeerMedia[fullscreenPeerId]?.cameraOff}
              />
            )}
          </div>
        </>
          ...
          <div className="min-h-0 flex-1 touch-pan-y overflow-y-auto overscroll-y-contain p-3 [-webkit-overflow-scrolling:touch]">
            <div className={`grid gap-3 ${gridCols(tileCount)}`}>
              <div
                ...
                className={`cursor-grab active:cursor-grabbing ${isDraggingLocal ? 'opacity-80' : ''}`}
              >
                <PeerTile
                  peerId="LOCAL"
                  stream={localStream}
                  ...
                  position={isDraggingLocal ? localDragPos || undefined : undefined}
                />
              </div>
              {remoteEntries.map(([peerId, stream]) => {
                ...
                return (
                  <PeerTile
                    ...
                    isFullscreen={fullscreenPeerId === peerId}
                    onFullscreenToggle={() =>
                      setFullscreenPeerId(
                        fullscreenPeerId === peerId ? null : peerId
                      )
                    }
                  />
                )
