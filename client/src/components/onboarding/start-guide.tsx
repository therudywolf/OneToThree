...
export function StartGuide({ onComplete }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-void/95 px-4 font-mono backdrop-blur-sm">
      <div className="relative w-full max-w-lg border border-border-strong bg-void p-6 shadow-[0_0_50px_rgba(0,0,0,0.5)]">
          ...
          {SEQUENCES.map((_, i) => (
            <div
              ...
              className={`flex-1 transition-all duration-300 ${
                i <= sequence ? 'bg-neon-red shadow-[0_0_8px_rgba(255,0,0,0.4)]' : 'bg-void'
              }`}
            />
          ))}
        </div>
        ...
        <div className="flex items-center gap-3">
          {sequence > 0 && (
            <button
              ...
              className="flex h-10 items-center border border-border-strong bg-void px-4 text-[10px] uppercase tracking-widest text-text-muted transition-all hover:border-neon-cyan hover:text-neon-cyan"
            >
              {t('common.back')}
            </button>
          )}
          ...
          <button
            type="button"
            onClick={onComplete}
            className="ml-auto text-[9px] uppercase tracking-widest text-text-muted/70 hover:text-neon-red transition-colors"
          >
            {t('common.skip')}
          </button>
        </div>
