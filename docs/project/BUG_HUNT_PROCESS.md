...
1. **Reproduce / confirm** against the code (and prod where safe — `server.example`,
   `onetothree.ru` is a no-real-user test environment; the DB is reachable via
   `ssh server.example "docker exec -i forestmessenger-db-1 psql -U forest -d forest"`).
...
6. **Deploy at a group boundary** (don't rebuild after every single commit):
   ```
   ssh server.example '~/stacks/onetothree.ru/deploy.sh'
   ```
