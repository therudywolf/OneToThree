...
| Object storage (MinIO) | optional — `fs` driver is the default | installer question; `media` compose profile | — |
| LiveKit SFU | bundled (`calls` profile) or external | installer question; one UDP port (7882) | coturn is not bundled — see Sprint 3 |
...
- [x] Local-filesystem media driver (`MEDIA_DRIVER=fs`, the installer default), so `FEATURE_MEDIA` is on **without** MinIO: objects are files under `MEDIA_FS_ROOT`, and the API serves them from its own origin over HMAC-signed expiring URLs — the same capability model as a presigned S3 URL. Every existing call site keeps working through an `S3Client`-shaped adapter (`server/src/lib/fs-s3-adapter.ts`).
- [x] **Bundled LiveKit `calls` profile** so `FEATURE_CALLS` works one-click: `livekit/livekit-server` behind the `calls` profile, keys generated by the installer, config rendered to `infra/lite/livekit.yaml`, signalling proxied by Caddy at `/livekit` on the existing origin, and **one** published UDP port (7882) instead of a range. coturn is deliberately NOT bundled: LiveKit's embedded TURN needs its own certificate to be useful, and a call that cannot get through still falls back to the encrypted WebSocket relay.
- **Exit:** media works on a 1-container Lite install; calls work without hand-rolling a LiveKit. ✅
  Verified live against a `local` install: 9/9 media checks (upload, download,
  range, and the four ways a forged link must fail) and 5/5 SFU checks (Caddy
  route, key match, forged token rejected, unauthenticated rejected, room
  created). Media itself flowing between two cameras is **not** covered — that
  needs two machines.
...
| MinIO | необязателен — по умолчанию драйвер `fs` | вопрос установщика; профиль `media` | — |
| LiveKit SFU | встроенный (профиль `calls`) или свой | вопрос установщика; один UDP-порт (7882) | coturn не встроен — см. Спринт 3 |
