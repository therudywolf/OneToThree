 ...
 */
export function verifyNonceSignatureEcdsaP256(nonceUtf8, signatureInput, publicKeyJwkString) {
    let jwk;
    try {
    ...
    if (sig.length === 64) {
        ...
        vRaw.end();
        try {
            return vRaw.verify({ key: publicKey, dsaEncoding: 'ieee-p1363' }, sig);
        }
        catch {
            return false;
        }
...
export function safeEqualNonce(a, b) {
...
function decodeSignatureBuffer(signatureInput) {
