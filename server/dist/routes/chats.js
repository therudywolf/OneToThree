...
export const chatsRoutes = async (app) => {
    app.get('/', async (request, reply) => {
        ...
        const memberRows = chatIds.length === 0
            ? []
            : await db
                .select({
    ...
    app.post('/', async (request, reply) => {
        ...
        if (type === 'group_e2e') {
            ...
            const keyByUser = new Map(members.map((m) => [m.userId, m.encryptedGroupKey]));
            ...
            const [created] = await db.transaction(async (tx) => {
                ...
                if (!chat)
                    throw new Error('INSERT_CHAT_FAILED');
                await tx.insert(chatMembers).values(uniqueIds.map((uid) => ({
                    chatId: chat.id,
                    userId: uid,
